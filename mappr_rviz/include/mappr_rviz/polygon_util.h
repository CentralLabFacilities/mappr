...
geometry_msgs::Point compute2DPolygonCentroid(const std::vector<geometry_msgs::Point> vertices)
{
...
bool isInsidePolygon(const std::vector<geometry_msgs::Point> points, double x, double y)
{
  int cross = 0;
...
unsigned char* makeMapPalette()
{
  ...
  {
    *palette_ptr++ = color.red();    // red
    *palette_ptr++ = color.green();  // green
    *palette_ptr++ = color.blue();   // blue
    *palette_ptr++ = color.alpha();  // alpha
  }
